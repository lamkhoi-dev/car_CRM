  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-5 gap-3">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium">Giá xe 18 chỗ (VNĐ)</label>
          <input type="number" value={form.price18Seat} onChange={(e) => setForm({ ...form, price18Seat: Number(e.target.value) })}
            className="w-full rounded-lg border border-border bg-secondary px-3 py-2 text-sm focus:border-primary focus:outline-none" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium">Giá xe 29 chỗ (VNĐ)</label>
          <input type="number" value={form.price29Seat} onChange={(e) => setForm({ ...form, price29Seat: Number(e.target.value) })}
            className="w-full rounded-lg border border-border bg-secondary px-3 py-2 text-sm focus:border-primary focus:outline-none" />
        </div>
      </div>
      <div>
                  ...
                  <div key={pkg.id} className="rounded-xl bg-card p-4 card-shadow">
                    ...
                    <div className="grid grid-cols-5 gap-2 text-xs">
                      ...
                      <div className="rounded-lg bg-secondary p-2">
                        ...
                        <div className="font-semibold">{formatVND(pkg.price18Seat)}</div>
                        <div className="text-muted-foreground mt-0.5">+{formatVND(pkg.overagePerKm18Seat)}/km · +{formatVND(pkg.overagePerHour18Seat)}/h</div>
                      </div>
                      <div className="rounded-lg bg-secondary p-2">
                        <div className="text-muted-foreground">29 chỗ</div>
                        <div className="font-semibold">{formatVND(pkg.price29Seat)}</div>
                        <div className="text-muted-foreground mt-0.5">+{formatVND(pkg.overagePerKm29Seat)}/km · +{formatVND(pkg.overagePerHour29Seat)}/h</div>
                      </div>
                    </div>
