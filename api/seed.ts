...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const mode = (req.query.mode as string) || 'reset';

  try {
    const db = await getDb();

    // ─── MODE: UPDATE (no reset) ─────────────────────────
    if (mode === 'update') {
      ...
      for (const [to, price29] of Object.entries(routePrice29Map)) {
        await db.collection('routes').updateOne({ to }, { $set: { price29Seat: price29 } });
      }
      ...
      await db.collection('pricingPackages').updateMany(
        { price16Seat: { $exists: true } },
        [
          { $set: { price18Seat: '$price16Seat', overagePerKm18Seat: '$overagePerKm16Seat', overagePerHour18Seat: '$overagePerHour16Seat', weekendSurcharge18Seat: '$weekendSurcharge16Seat' } },
      ...
      // 3. Rename 16-seat vehicles → 18 seat
      const transitUpdate = await db.collection('vehicles').updateOne(
        { name: { $regex: /Ford Transit/i } },
        { $set: { name: 'Ford Transit Luxury 18 chỗ', description: 'Ford Transit Luxury 18 chỗ — lý tưởng cho du lịch nhóm, team building, đưa đón sự kiện.', seatCategory: '18_cho', seats: 18 } }
      );
      const sprinterUpdate = await db.collection('vehicles').updateOne(
        { name: { $regex: /Sprinter/i } },
        { $set: { name: 'Mercedes Sprinter VIP', description: 'Mercedes Sprinter VIP — xe sang 18 chỗ dành cho đoàn khách VIP, nội thất da cao cấp, mini bar.', seatCategory: '18_cho', seats: 18 } }
      );
      results.push(`Vehicles: Transit updated=${transitUpdate.modifiedCount}, Sprinter updated=${sprinterUpdate.modifiedCount}`);

      // 4. Insert 2 new 29-seat vehicles (if not exist)
      const existing29 = await db.collection('vehicles').countDocuments({ seatCategory: '29_cho' });
      if (existing29 === 0) {
        const new29Vehicles = [
          {
            name: 'Hyundai County 29 chỗ', brand: 'Hyundai', type: 'van', year: 2024, color: 'Trắng Ngà',
            description: 'Hyundai County 29 chỗ — xe khách cỡ trung phổ biến nhất Việt Nam. Phù hợp cho du lịch đoàn, team building, sự kiện.',
            pricePerDay: 4000000, pricePerHour: 600000, seatCategory: '29_cho', selfDrivePrice: 0, chauffeurIncluded: true,
            licensePlate: '51B-290.29',
            images: ['https://res.cloudinary.com/dpr6zwanv/image/upload/v1772344379/carCRM/vehicles/xe_A/xe_A__3_.jpg', 'https://res.cloudinary.com/dpr6zwanv/image/upload/v1772344364/carCRM/vehicles/xe_A/xe_A__1_.jpg'],
            features: ['29 Seats', 'AC', 'USB Ports', 'Luggage Compartment', 'Reclining Seats'],
            seats: 29, transmission: 'Manual', fuel: 'Diesel', rating: 4.5, available: true, createdAt: '2026-01-25T00:00:00Z',
            packages: [
              { id: 'pkg-s19-1', serviceTypeSlug: 'hourly_4h', name: 'Gói 4 tiếng', price: 1800000, durationHours: 4, maxKm: 50, isActive: true },
              { id: 'pkg-s19-2', serviceTypeSlug: 'hourly_8h', name: 'Gói 8 tiếng', price: 2200000, durationHours: 8, maxKm: 100, isActive: true },
              { id: 'pkg-s19-3', serviceTypeSlug: 'daily', name: 'Theo ngày', price: 4000000, durationHours: 12, maxKm: 200, isActive: true },
          ...
          {
            name: 'Thaco Town TB82S 29 chỗ', brand: 'Thaco', type: 'van', year: 2024, color: 'Xanh Dương',
            description: 'Thaco Town TB82S — xe khách 29 chỗ cao cấp, ghế ngồi êm ái, hệ thống giải trí hiện đại. Lý tưởng cho tour du lịch dài ngày.',
            pricePerDay: 4500000, pricePerHour: 700000, seatCategory: '29_cho', selfDrivePrice: 0, chauffeurIncluded: true,
            licensePlate: '51B-290.88',
            images: ['https://res.cloudinary.com/dpr6zwanv/image/upload/v1772344380/carCRM/vehicles/xe_B/xe_B__1_.jpg', 'https://res.cloudinary.com/dpr6zwanv/image/upload/v1772344383/carCRM/vehicles/xe_B/xe_B__3_.jpg'],
            features: ['29 Seats', 'Premium AC', 'LCD Entertainment', 'Large Luggage', 'Curtains'],
            seats: 29, transmission: 'Manual', fuel: 'Diesel', rating: 4.6, available: true, createdAt: '2026-01-25T00:00:00Z',
            packages: [
              { id: 'pkg-s20-1', serviceTypeSlug: 'hourly_4h', name: 'Gói 4 tiếng', price: 2000000, durationHours: 4, maxKm: 50, isActive: true },
              { id: 'pkg-s20-2', serviceTypeSlug: 'hourly_8h', name: 'Gói 8 tiếng', price: 2500000, durationHours: 8, maxKm: 100, isActive: true },
              { id: 'pkg-s20-3', serviceTypeSlug: 'daily', name: 'Theo ngày', price: 4500000, durationHours: 12, maxKm: 200, isActive: true },
